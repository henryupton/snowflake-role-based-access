...
module "parse_schema_wildcards" {
  ...
  payload = var.row_access_policies
...
data "snowflake_row_access_policies" "row_access_policies" {
...
module "resolve_wildcards" {
  ...
  payload    = module.parse_input.return
  candidates = data.snowflake_row_access_policies.row_access_policies[each.key].row_access_policies
}
...
resource "snowflake_grant_privileges_to_role" "future" {
  ...
  on_schema_object {
    future {
      object_type_plural = upper("row access policies")
