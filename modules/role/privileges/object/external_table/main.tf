...
data "snowflake_external_tables" "external_tables" {
...
module "resolve_wildcards" {
  ...
  payload    = module.parse_input.return
  candidates = data.snowflake_external_tables.external_tables[each.key].external_tables
}
