...
data "snowflake_event_tables" "event_tables" {
...
module "resolve_wildcards" {
  ...
  payload    = module.parse_input.return
  candidates = data.snowflake_event_tables.event_tables[each.key].event_tables
}
...
resource "snowflake_grant_privileges_to_role" "future" {
  ...
  on_schema_object {
    future {
      object_type_plural = upper("event tables")
