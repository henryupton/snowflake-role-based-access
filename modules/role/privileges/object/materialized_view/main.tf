...
module "parse_schema_wildcards" {
  ...
  payload = var.materialized_views
...
data "snowflake_materialized_views" "materialized_views" {
...
module "resolve_wildcards" {
  ...
  payload    = module.parse_input.return
  candidates = data.snowflake_materialized_views.materialized_views[each.key].materialized_views
}
...
resource "snowflake_materialized_view_grant" "grant" {
  ...
  materialized_view_name    = each.value.name
...
resource "snowflake_materialized_view_grant" "futures" {
