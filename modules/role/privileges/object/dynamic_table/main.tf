...
data "snowflake_dynamic_tables" "dynamic_tables" {
...
module "resolve_wildcards" {
  ...
  payload    = module.parse_input.return
  candidates = data.snowflake_dynamic_tables.dynamic_tables[each.key].dynamic_tables
}
...
resource "snowflake_grant_privileges_to_role" "grant" {
  ...
  on_schema_object {
    object_type = upper("dynamic table")
...
resource "snowflake_grant_privileges_to_role" "future" {
  ...
  on_schema_object {
    future {
      object_type_plural = upper("dynamic tables")
