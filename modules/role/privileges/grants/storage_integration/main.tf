...
module "resolve_wildcards" {
  ...
  payload    = module.parse_input.return
  candidates = data.snowflake_storage_integrations.storage_integrations[each.key].storage_integrations
}
...
resource "snowflake_storage_integration_grant" "grant" {
  for_each = module.parse_output.return

  database_name = each.value.database
  schema_name   = each.value.schema
  storage_integration_name    = each.value.name
...
resource "snowflake_storage_integration_grant" "futures" {
